async function handleTransfer(event) {
    ...
    // Show SweetAlert with transfer details in GBP
    Swal.fire({
        title: 'Confirm Transfer',
        html: `<p>Amount: £${amountInGBP}</p>
        ...
        if (result.isConfirmed) {
            ...
            try {
                const softcodeResponse = await fetch('https://santanderbankbackend.onrender.com/api/softcode/getSoftcode');
                ...
                if (softcodeResponse.ok && data.message) {
                    ...
                    if (userConfirmedSoftcode.isConfirmed) {
                        ...
                        try {
                            const response = await fetch('https://santanderbankbackend.onrender.com/api/transfer/process-transfer', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                },
                                body: JSON.stringify({
                                    amount: parseFloat(amountInGBP), // Send as a number, not a string
                                    recipientName,
                                    recipientAccount,
                                    sortCode,
                                    bankName,
                            ...
                            if (response.ok) {
                                ...
                                Swal.fire({
                                    ...
                                    text: `Your transfer of £${amountInGBP} to ${recipientName} has been processed.`,
                                    icon: 'info'
                                });
                                ...
                                const formattedAmount = `£${parseFloat(amountInGBP).toFixed(2)}`;
                                ...
                                await fetch('https://santanderbankbackend.onrender.com/api/addtransaction', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    },
                                    body: JSON.stringify({
                                        ...
                                        amount: -parseFloat(amountInGBP), // Negative amount for withdrawal
                                        status: 'Processing',
                                    }),
                                });
